class Size {
  ...
  render() {
    ...
    if (this.resources.helpers.isPercentage(this.resources.settings.offset)) {
      ...
      style += `{{var}}val{{=}}{{i}}calc(math.div(100%, {{string-var}}columns{{/string-var}}) * {{string-var}}n{{/string-var}} - {{string-var}}offset${mediaPostfix}{{/string-var}}){{/i}}{{;}}\n`;
      ...
      style += `{{var}}val{{=}}math.div(100%, {{var}}columns) * {{var}}n{{;}}\n`;
